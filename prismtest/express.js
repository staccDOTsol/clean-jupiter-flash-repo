	...
	if (!thedoing) {
		...
		try {
			...
			if (tokenb) {
				...
				console.log(
					"amttotrade: " +
						(amountToTrade / 10 ** atokens[1].decimals).toString()
				);
				try {
...
new PublicKey(market.reserves[i].config.liquidityToken.mint*/ //)
										...
										console.log(
											"we wanna c > " +
												(parseInt(
													pubkey[0].account.data.parsed.info.tokenAmount.amount
												) +
													14) /
													10 ** 6
										);
										instructions.push(
											createTransferInstruction(
												...
												parseInt(
													pubkey[0].account.data.parsed.info.tokenAmount.amount
												) + 14 // + Math.ceil(JSBI.toNumber(solamis.routesInfos[0].outAmount)* 10 ** atokens[1].address)
											)
										);
...
var superconnection = new Connection(
	"https://rpc.helius.xyz/?api-key=440ba15e-d63b-4177-914f-c66c18400739"
);
