...
console.log('amttotrade: ' + (amountToTrade / 10 ** token.decimals).toString())
		...
		if (!routes[0]) return
		if (innn < routes[0].minimumReceived * 10 ** dec) {
				...
				try {
					...
					try {
						...
						if (true) {
							...
							let routes2 = prism.getRoutes((routes[abc].minimumReceived) / 1.015); // get routes based on from Token amount 10 USDC -> ? PRISM
							...
							if (true) {
								...
								try {
									for (var bca of [0,1,2]) {
										console.log(
											routes2[bca].minimumReceived > routes[abc].amountIn * 1.043
										);
										if (
											routes2[bca].minimumReceived > routes[abc].amountIn * 1.043 &&
											!doing
										...
										//	doing = true
											console.log(
												...
												token.address + " mod " +
													...
													"$ " +
													(routes[abc].minimumReceived).toString() +
													" outamnt " +
													tokenb.symbol +
													" " +
													tokenb.address
                ...
                new PublicKey(reserve.config.liquidityToken.mint)*/
											...
											instructions.push(
												createTransferInstruction(
													...
													).value[0].account.data.parsed.info.tokenAmount.amount //+ Math.ceil(solamis[0].minimumReceived * 0.8 * 10 ** token.decimals)
												)
											);
...
var result = await connection.sendTransaction(tx, [wallet]
	...
	console.log('tx1: ' + result)
		...
		//oldData[token.address + tokenb.address] = maybe.oldData; // load routes for tokens, tokenSymbol | tokenMint (base58 string)
		let routes2 = prism.getRoutes(routes[0].minimumReceived / 2); // get routes based on from Token amount 10 USDC -> ? PRISM
