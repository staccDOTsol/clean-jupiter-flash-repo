        ...
        if (goaccs.length < 300) {
          try {
            ...
            if (test.state.deactivationSlot > BigInt(159408000 * 2)) {
              ...
              if (acompare > lastcompare && acompare > arr1.length / 10){
                if (goaccs.length > 14){
...
try {
 //  oldData[tokenb.address + token.address] =    maybe2.oldData
   if (true){
    console.log(mod.toString()  + ' mod ' + (amountToTrade / 10 ** 6).toString() + '$ ' + routes[abc].amountOut.toString() + ' outamnt ' + tokenb.symbol + ' ' + tokenb.address)
