...
function openbookSwap(user, connection, settings, program, route, fromTokenAccount, toTokenAccount, fees, preTransaction, preSigners, hostFees, userOpenOrders, useT = null, disableFees = false) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const openbookProgram = new anchor_1.Program(serumIdl_1.IDL, new web3_js_1.PublicKey(types_1.OPENBOOK_PROGRAM_ID), provider);
        let { fromCoin, toCoin, side, marketConfig } = route.routeData;
        let market = marketConfig.market;
        let openOrders = null;
        if (!userOpenOrders) {
            let openOrdersArr = yield serum_1.OpenOrders.findForMarketAndOwner(connection, market.address, user, new web3_js_1.PublicKey(types_1.OPENBOOK_PROGRAM_ID));
        ...
        if (!openOrders) {
            ...
            preTransaction.add(yield serum_1.OpenOrders.makeCreateAccountTransaction(connection, market.address, user, account.publicKey, new web3_js_1.PublicKey(types_1.OPENBOOK_PROGRAM_ID)));
            let data = Buffer.from([0, 15, 0, 0, 0]);
            openOrders = account.publicKey;
            let initTx = new web3_js_1.TransactionInstruction({
                keys: [
                    {
                        pubkey: openOrders,
                        isSigner: false,
                ...
                programId: new web3_js_1.PublicKey(types_1.OPENBOOK_PROGRAM_ID),
                data,
            });
        ...
        return program.instruction.openbookSwap(new bn_js_1.default(route.amountIn * 10 ** fromCoin.decimals), new bn_js_1.default(route.minimumReceived * 10 ** toCoin.decimals), useT ? true : false, (side == "buy") ? new bn_js_1.default(0) : new bn_js_1.default(1), disableFees, new bn_js_1.default(hostFees), {
            accounts: {
                serumDexProgram: new web3_js_1.PublicKey(types_1.OPENBOOK_PROGRAM_ID),
...
exports.openbookSwap = openbookSwap;
