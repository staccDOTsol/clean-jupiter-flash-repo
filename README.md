# hi it's @staccoverflow

this doez everything below except

1. on prism
2. it arbs more than one pair per instance
3. it arbs with flashloans - you don't need money to make money

There are two modes:

1. solend (but they're presently not allowing flashloans cuz of an incident)
2. risk.lol protocol (a solend fork with some key differences ie. much less flashloaning fees and rewards for depositors as yield from flashloanin')

https://risklol.gitbook.io/risklol-an-equity-cooperative/risk.lol/lol-protocol
...
This bot is an open-source CLI tool that allows you to automate your crypto trading strategies on the Solana blockchain. The bot is currently written in JS and uses the [Prism SDK](https://docs.jup.ag/prism-core/prism-sdk/v1) to find routes and execute trades.
...
CLI UI currently displays a simulated profit chart and latency chart. The latency chart shows you the time taken to compute routes with Prism SDK.
...
![](https://github.com/arbprotocol/solana-prism-bot/blob/main/.gifs/wizard.gif)

![](https://github.com/arbprotocol/solana-prism-bot/blob/main/.gifs/bot.gif)

![](https://github.com/arbprotocol/solana-prism-bot/blob/main/.gifs/history.gif)
...
```bash
$ git clone https://github.com/arbprotocol/solana-prism-bot && cd solana-prism-bot
$ yarn
```
...
`[L]` - show/hide latency chart (of Prism `computeRoutes()`)
...
Simple percentage slippage. The percentage slippage is calculated by the Prism SDK.
...
Arb Prism Bot is an open source project and contributions are welcome.
