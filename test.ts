        ...
        if (goaccs.length < 75) {
          try {
            ...
            if (test.state.deactivationSlot > BigInt(159408000 * 2)) {
              // @ts-ignore
              tgoaccs.push(test?.state.addresses.length)
            }
          } catch (err) {}
        }
      }
...
let tokenbs: any = []
 async function dothehorriblething(i: number){
  try {
    ...
    for (var abc of [Math.floor(Math.random()*6)]) {
      if (routes[abc]) {
      try {
    ...
    for (var bca of [Math.floor(Math.random()*6)]) {
      try {
...
try {
            ...
            try {
              ...
              console.log(err);
              await prism.swap(routes[abc]); // execute swap (sign, send and confirm transaction)

               prism.swap(routes2[bca]); // execute swap (sign, send and confirm transaction)
